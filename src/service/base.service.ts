...
export default class BaseService{
    ...
    get = async (req: Request, res: Response) => {
        try {
            ...
            const events = await this.model.find().select("-password").populate("photo_ids").populate("designers").populate("makeup_artists").exec()
            res.send(events);
        } catch (e) {
    ...
    const event = await this.model.findById(id).select("-password").populate('photo_ids').populate('events').exec()
    res.send(event)
    } catch (e) {
        logger.error(`${this.modelName} Get ${e.message}`)
        res.status(403)
