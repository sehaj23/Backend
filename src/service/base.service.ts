...
export default class BaseService {
    ...
    getId = async (id: string) => {
         return await this.model.findById(id).select("-password").populate("profile_pic").populate({path:"employees",populate: { path: 'photo' }}).populate("user_id").populate("salon_id").populate("designer_id").populate("makeup_artist_id").populate("events")  //.populate({
