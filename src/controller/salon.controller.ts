...
export default class SalonController extends BaseController {
  ...
  salonSettings = controllerErrorHandler(
  ...
  addSalonService = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const d = req.body;
      const salon_id = req.params.id;
  ...
  deleteSalonService = controllerErrorHandler(
    async (req: Request, res: Response) => {
  ...
  getService = controllerErrorHandler(async (req: Request, res: Response) => {
    ...
    const filterService = services.filter((service: ServiceI) => {
      ...
      const filterOptions = options.filter((opt: OptionI) => {
        if (req.query.gender) {
          if (
            opt.gender !== req.query.gender &&
            req.query.gender !== "both" &&
            opt.gender !== "both"
          ) {
            return false;
          }
  ...
  updateService = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const d = req.body;
      const id = req.params.id;
      const sid = req.params.sid;
  ...
  addSalonEmployee = controllerErrorHandler(
    async (req: Request, res: Response) => {
  ...
  deleteSalonEmployee = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const _id = req.params.id;
      ...
      const newSalon = await this.service.deleteSalonEmployee(
        _id,
        eid,
        vendor_id
      );
  ...
  editSalonEmployee = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const v = req.body;
      const salon_id = req.params.id;
  ...
  putProfilePic = controllerErrorHandler(
    async (req: Request, res: Response) => {
  ...
  addSalonEvent = controllerErrorHandler(
    async (req: Request, res: Response) => {
      ...
      if (designerEvent === null) {
        ...
        res.status(400);
        res.send({
          message: `Not able to update event: eventid -, event_id: ${d.event_id}`,
        });
        return;
      }
  ...
  getSalonInfoByID = controllerErrorHandler(
    async (req: Request, res: Response) => {
      //TODO:validator
      ...
      let sr: string = await SalonRedis.get(salonId, filter);
      if (sr !== null) {
          ...
          if (user) {
            ...
            user.favourites.map((e) => {
              if (e.toString() === salonId.toString()) {
                //@ts-ignore
                sr.salon.userFavourite = true;
              }
            });
            return res.send(sr)
          }
        }
        return res.send(JSON.parse(sr));
      }
      const salonReq = this.service.getSalonInfo(
        salonId,
        centerPoint,
        getDistance
      );
      ...
      let [salon, reviews, user, promocodes] = await Promise.all([
        salonReq,
        reviewsReq,
        userReq,
        promoCodeReq,
  ...
  getSalonInfo = controllerErrorHandler(async (req: Request, res: Response) => {
    ...
    const [salon, reviews, user, promocodes] = await Promise.all([
      salonReq,
      reviewsReq,
      userReq,
      promoCodeReq,
  ...
  getRecomendSalon = controllerErrorHandler(
    async (req: Request, res: Response) => {
      let salons;
      let getDistance = false;
      //  const sr = await SalonRedis.get('Salons')
  ...
  getHomeServiceSalon = controllerErrorHandler(
    async (req: Request, res: Response) => {
      let salons;
      const q = req.query;
      //TODO: store location of User
  ...
  // })
  getSearchResult = controllerErrorHandler(
    async (req: Request, res: Response) => {
      //TODO:Validato
  ...
  //     res.status(200).send(salon)
  // })
  getSalonNearby = controllerErrorHandler(
    async (req: Request, res: Response) => {
      var centerPoint = {};
      //TODO: store location of User
      const q = req.query;
  ...
  getSalonDistance = controllerErrorHandler(
    async (req: Request, res: Response) => {
      var centerPoint = {};
      //TODO: store location of User
      //@ts-ignore
  ...
  getSalonCategories = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const _id = req.params.id;
      const data = new Array();
  ...
  getSalonReviews = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const _id = req.params.id;
      const q = req.query;
  ...
  postSalonReviews = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const _id = req.params.id;
  ...
  checkPostReviews = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const _id = req.params.id;
      //@ts-ignore
      ...
      if (check == null) {
        res.status(400);
        res.send({
          message: `User Cant Post Review,No Previous Bookings Found`,
          success: "false",
        });
        return;
      }
  ...
  getSalonCategory = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const phrase = req.query.phrase as string;
      let home: boolean;
      if (req.query.home) {
  ...
  getSearchedService = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const phrase = req.query.phrase as string;
      let home: boolean;
      if (req.query.home) {
  ...
  getRatings = controllerErrorHandler(async (req: Request, res: Response) => {
    ...
    const [rating, reviews, user] = await Promise.all([
      ratingReq,
      reviewsReq,
      userReq,
    ]);
  ...
  getNameIDRatingProfile = controllerErrorHandler(
    async (req: Request, res: Response) => {
  ...
  getUnapprovedSalon = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const q = req.query;
  ...
  getSalonPhoto = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const id = req.params.id;
  ...
  clearRedisSalonByPromo = controllerErrorHandler(
    async (req: Request, res: Response) => {
  ...
  getSalonByPromo = controllerErrorHandler(
    async (req: Request, res: Response) => {
      const id = req.params.id;
      const q: any = req.query;
      let getDistance = false;
      ...
      if (promoGetSalon == null) {
        const promo = (await this.promoCodeService.getPromoById(
          id
        )) as PromoCodeSI;

        const salon = await this.service.getSalonByIds(
          promo.salon_ids,
          q,
          getDistance
        );
  ...
  getDistanceInPairs = controllerErrorHandler(
    async (req: Request, res: Response) => {
