...
export default class PromoCodeController extends BaseController {
    ...
    discountApplicable = controllerErrorHandler(async (req: Request, res: Response) => {
        ...
        if (promoCode === null) throw new ErrorResponse(`Promo code not found`)
        if (promoCode.active === false) throw new ErrorResponse(`Promo code not active anymore`)
        const currentDateTime = moment(Date.now())
        // time check
        if(!currentDateTime.isBefore(promoCode.expiry_date_time)) throw new ErrorResponse(`Promo code is expired`)
        if(promoCode.time_type === 'Custom'){
            
            const currentDay = currentDateTime.day()
            const DAYS = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
            if(!promoCode.custom_time_days.includes(currentDay))throw new ErrorResponse(`This promo code is not valid on ${DAYS[currentDay]}`)
            if(!currentDateTime.isBetween(promoCode.custom_time_start_time, promoCode.custom_time_end_time)) throw new ErrorResponse(`This promo code is valid between ${promoCode.custom_time_start_time} and ${promoCode.custom_time_end_time}`)
        }
        if (promoCode.user_ids && promoCode?.user_ids?.length > 0) {
            if (!promoCode.user_ids.includes(userId)) throw new ErrorResponse(`Current user doe not support this coupon code`)
        }
        ...
        if ((promoCode.max_usage !== -1 || !promoCode.max_usage) && promoCode.max_usage <= promoUserCount.length) throw new ErrorResponse(`You have exceeded the max usage: ${promoCode.max_usage}`)
        // check for the salon
        const cart = await this.cartService.getId(cart_id) as CartSI
        if (cart === null) throw new ErrorResponse(`Not able find cart with this id`)
        // minimum bill check
        if(cart.total < promoCode.minimum_bill)throw new ErrorResponse(`You cannot apply this coupon code. Minimum bill should be ${promoCode.minimum_bill}`)
        if (promoCode.salon_ids && promoCode.salon_ids?.length > 0) {
            if (!promoCode.salon_ids.includes(cart.salon_id)) throw new ErrorResponse(`This coupon code is not applied on this salon`)
        }
        ...
        if (promoCode.categories && promoCode.categories.length > 0) {
            const salon = await this.salonService.getId(cart.salon_id) as SalonSI
            if (salon === null) throw new ErrorResponse(`Salon not found from the cart`)
